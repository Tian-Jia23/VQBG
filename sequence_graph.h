...
//                                }
    ...
    std::vector<std::vector<size_t>> compare_node_reference1(std::vector<std::string>& refer,std::vector<Node>& nodes) {
        std::vector <std::vector<size_t>> sames;
        std::vector <size_t> same;
//        std::ofstream outfile("3ZIKV_gra_result_bu.txt");
        std::ofstream outfile("10_sim_node_sequence.txt");
        std::ofstream nodefile("10_sim_node.txt");
//        std::ofstream outfile("15Z_node_results.txt");
//        std::ofstream outfile("10HCV_node_results.txt");
        std::map<size_t,size_t> used_sim_n;
...
//                if (node.sequence.length() > 2){
                if (refer[i].find(node.sequence) != std::string::npos) {
                    true_nodes[node.id]++;
                    used_sim_n[node.id]++;
                    nodefile << node.id << "(" << node.coverage << ")  ";
                    if (node.parents.size() == 0) {
                        same.push_back(node.id);
                        str = str + node.sequence;
                    } else {
                        same.push_back(node.id);
                        str = str + node.sequence.substr(g_kmer_length - 1);
                    }
                }
//                }
                n++;
        ...
        int e = 0;
        for (const Node &its: nodes) {
            if (used_sim_n.find(its.id)==used_sim_n.end()){
                cout << its.id << endl;
                e++;
            }
        }
...
//                                            cout << i << " : " << sim_graph1[i].children[j] <<endl;
    ...
    vector<pair<vector<size_t>,double>> find_paths(std::vector<Node>& result,vector<Node>& sim_graph1,std::vector<std::map<size_t,size_t>>& same_position_sim){
            ...
            for (int i = 0; i < sim_graph1.size(); ++i) {
                while(sim_graph1[i].parents.size() == 0 && sim_graph1[i].children.size()!= 0 && sim_graph1[i].coverage > 0.08){
                    vector<double> path_node_coverage;
                    if (sim_graph1[i].coverage < 0.02 && use_node[i] >= 1){
                        break;
                    }
...
//            cout << " not used nodes num : " << sim_graph1.size() - use_node.size() <<endl;
            for (int i = 0; i < sim_graph1.size(); ++i) {//根据剩余没用过的节点再找路
                vector<size_t> path2;
...
//            cout << paths.size() <<endl;
    ...
    vector<set<size_t>> densityClustering(const vector<Node>& points, double& tolerance) {//将点按照丰度聚类
        ...
        for (int i = 0; i < points.size(); ++i) {
            ...
            set<size_t> newCluster;
            newCluster.insert(points[i].id);
            clustered[i] = true;

            // Check against remaining points
            for (int j = i + 1; j < points.size(); ++j) {
                if (!clustered[j] && abs(points[i].coverage - points[j].coverage) <= tolerance) {
                    newCluster.insert(points[j].id);
                    clustered[j] = true;
                }
            }
        ...
        std::ofstream cluster_file("6polio_cluster.txt");
        // Output clusters
        for (int i = 0; i < clusters.size(); ++i) {
            cluster_file << "Cluster " << i + 1 << ": ";
            for (auto it : clusters[i]) {
                cluster_file << it << "(" << points[it].coverage << ")  ";
            }
            cluster_file << endl;
        }
        cluster_file.close();
...
//                cout << cut_position[cut_position.size()-1] <<endl;
    ...
    void similarity_between_roads(vector<pair<string,double>>& paths_str){
...
//               cout << i << " - " << j << " : " << num <<endl;
                if (num < paths_str[i].first.length()*0.015){
                    similar.insert(j);
                }
...
//            paths_str[i].second = std::round(paths_str[i].second * 1000.0) / 1000.0;
        ...
        if (paths_str.size() >= 10){
            for (auto it : similar) {
                paths_str.erase(paths_str.begin()+it);
            }
        }
    }

    void output_graph(KmerMap& kmerMap, kmer_int_type_t& seed_kmer,int& average, std::vector<std::string>& refer,std::vector<std::string>& data){
...
//        cout << trunk_str <<endl;
       ...
       //拓扑排序
        ...
        vector<vector<size_t>> sim_gra = compression_nodes(kmerMap,result,after_gra2,layer_nodes,sim_layer,layer_sim_node,data);
...
//        std::vector<std::vector<size_t>> sames = compare_node_reference2(refer,sim_gra,result);
        ...
        //删除相似度高的contig
        similarity_between_roads(paths_str);
